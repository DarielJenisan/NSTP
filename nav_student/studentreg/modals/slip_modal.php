<!-- Slip Modal -->
<div class="modal fade" id="SlipModal" tabindex="-1" aria-labelledby="SlipModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" style="max-width: 930px;">
        <div class="modal-content" style="background-color: transparent; border: none; box-shadow: none;">
            <div class="modal-header" style="background-color: transparent; border: none; box-shadow: none;">
                <h5 class="modal-title" id="SlipModalLabel">Slip</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body centered-content" id="slipContent" style="background-color: transparent; border: none; box-shadow: none; padding: 0;">
                <!-- Content will be dynamically populated here -->
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
            </div>
        </div>
    </div>
</div>
